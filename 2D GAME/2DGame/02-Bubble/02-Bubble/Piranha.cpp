...
void Goomba::init(const glm::ivec2 &tileMapPos, ShaderProgram &shaderProgram)
{
	...
	spritesheet.loadFromFile("images/spritePlanta.png", TEXTURE_PIXEL_FORMAT_RGBA);
	sprite = Sprite::createSprite(glm::ivec2(32, 64), glm::vec2(0.25, 0.25), &spritesheet, &shaderProgram);
	sprite->setNumberAnimations(1);

	sprite->setAnimationSpeed(STATE, (nose)); //CAMBIAR ESTO 
	

	sprite->changeAnimation(0);
	tileMapDispl = tileMapPos;
	sprite->setPosition(glm::vec2(float(tileMapDispl.x + posENEMY.x), float(tileMapDispl.y + posENEMY.y))); //CAMBIAR ESTO
